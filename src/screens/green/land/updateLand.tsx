...
import { updateLandAction } from "../../../services/land/actions";
...
const UpdateLand = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setImage(`${MEDIA.SELF}?id=${land?.imgId}`)
        }
        fetchData();
    }, []);
    ...
    const handleSaveLand = async () => {
        ...
        if (image) {
            // @ts-ignore
            req.append('image', { uri: image, name: 'image.jpg', type: 'image/jpeg' });
        }
        req.append('userId', userId);
        req.append('id', land.id)
        ...
        try {
            ...
            const res = await dispatch(updateLandAction(req));
            if (res.payload) {
                setLoading(false);
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <ScrollView >
                ...
                <View style={styles.body}>
                    <Text style={styles.header}>CẬP NHẬT THÔNG TIN CHO KHU VƯỜN NHỎ CỦA BẠN</Text>
                    <View style={{}}>
