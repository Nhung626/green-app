...
import { likeCommentAction, unlikeCommentAction } from '../../../services/comment/actions';
import SubComment from './subComment';
import AddComment from './addComment';
import { searchGardenAction } from '../../../services/garden/actions';
import { useFocusEffect } from '@react-navigation/native';
import { MEDIA } from '../../../constants/api';
import { deleteCommentPostAction, likeCommentPostAction, unlikeCommentPostAction } from '../../../services/commentpost/actions';
...
const CommentPost = ({ data }) => {
    ...
    const handleLike = async () => {
        try {
            const res = await dispatch(likeCommentPostAction({ commentPostId: comment.id }))
            if (res.payload) {
    ...
    const handleUnLike = async () => {
        try {
            const res = await dispatch(unlikeCommentPostAction({ commentPostId: comment.id }))
            if (res.payload) {
                setIsLike(false);
                setLikeCount(likeCount - 1);
                setLoading(false);
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            const res = await dispatch(deleteCommentPostAction({ 'id': comment.id }));
                            if (res.payload) {
                                navigation.goBack();
    ...
    return (
        <View style={{ marginVertical: 5, }}>
            <View style={{ flexDirection: 'row', justifyContent: 'flex-start' }}>
                ...
                <View style={{ marginLeft: 5, backgroundColor: theme.color_2, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 10, marginRight: 10 }}>
